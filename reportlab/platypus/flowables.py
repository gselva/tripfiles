...
def _hmodel(s0,s1,h0,h1):
    ...
    # h = a/s**2 + b/s
    a11 = 1./s0**2 if s0 > 0 else 1.  #selva if's added on 24 nov 2010 to resolve zerodivison error
    a12 = 1./s0 if s0 > 0 else 1.
    a21 = 1./s1**2 if s1 > 0 else 1.
    a22 = 1./s1 if s1 > 0 else 1.
    det = a11*a22-a12*a21
    b11 = a22/det if det > 0 else a22
    b12 = -a12/det if det > 0 else -a12
    b21 = -a21/det if det > 0 else -a21
    b22 = a11/det if det > 0 else a11
    a = b11*h0+b12*h1
    b = b21*h0+b22*h1
    return a,b
    ...
    def wrap(self,availWidth,availHeight):
        ...
        if (mode=='error' and (W>maxWidth+_FUZZ or H>maxHeight+_FUZZ)):
            ...
            def func(x):
                ...
                if x > 0: #selva update for zerodivision error, 24 nov 2010
                    W /= x
                    H /= x
                return W, H
            W0 = W
    ...
    def drawOn(self, canv, x, y, _sW=0):
        ...
        if ss:
            ...
            if truncate:
                ...
                canv.translate(x,y)
                x=y=0
                scale = scale if scale > 0 else 1 # selva, if added or zerodivision error, 24 nov 2010
